<?php

namespace Modules\Socle\Filament\Resources\UtilisateurResource\Pages;

use Filament\Actions;
use Filament\Resources\Pages\ManageRecords;
use Filament\Support\Enums\Alignment;
use Modules\Socle\Filament\Concerns\TitreLisible;
use Modules\Socle\Filament\Resources\UtilisateurResource;
use Modules\Socle\Models\JournalAudit;
use Modules\Socle\Models\Utilisateur;

class ManageUtilisateurs extends ManageRecords
{
    // Filament capitalise chaque mot du libelle pluriel pour en
    // faire le titre : « Corps De Metier » la ou le menu et le fil
    // d'Ariane disent « Corps de metier ». Voir le trait.
    use TitreLisible;

    protected static string $resource = UtilisateurResource::class;

    public function getBreadcrumbs(): array
    {
        return [
            route('filament.admin.pages.dashboard') => 'Accueil',
            '' => 'Utilisateurs',
        ];
    }

    protected function getHeaderActions(): array
    {
        return [
            Actions\CreateAction::make()
                ->visible(fn () => auth()->user()->can('ajouter_utilisateur'))
                ->modalHeading('Nouvel utilisateur')
                ->modalWidth('3xl')
                ->createAnother(false)
                ->modalSubmitActionLabel('Enregistrer')
                ->modalCancelActionLabel('Fermer')
                ->modalFooterActionsAlignment(Alignment::End)
                ->stickyModalHeader()
                ->stickyModalFooter()
                ->after(fn (Utilisateur $record) => JournalAudit::enregistrer(
                    'Création utilisateur',
                    'SOCLE',
                    'Utilisateur',
                    $record->id,
                    ['email' => $record->email, 'roles' => $record->roles->pluck('name')->all()],
                )),
        ];
    }
}
